...
for file in tqdm(files):
	...
	if os.path.isfile(full_path) or processed_images < target:
		...
		BUFFER = {'JPEG_Planar': {'PSNR':[], 'SSIM':[], 'BPP':[]},
					'BRAHIMI_Planar_Round': {'PSNR':[], 'SSIM':[], 'BPP':[]},
					'OLIVEIRA_Planar_Round': {'PSNR':[], 'SSIM':[], 'BPP':[]},
					'RAIZA_Planar_Round': {'PSNR':[], 'SSIM':[], 'BPP':[]},
					'BRAHIMI_Planar_Ceil': {'PSNR':[], 'SSIM':[], 'BPP':[]},
					'OLIVEIRA_Planar_Ceil': {'PSNR':[], 'SSIM':[], 'BPP':[]},
					'RAIZA_Planar_Ceil': {'PSNR':[], 'SSIM':[], 'BPP':[]}}
		...
		for QF in quantization_factor:
			...
			QPhiBrahimiForward = np2_round(divide(QOliveira, ZB_tiled))
			QPhiBrahimiBackward = np2_round(multiply(QOliveira, ZB_tiled))
			...
			BUFFER['BRAHIMI_Planar_Round']['PSNR'].append(peak_signal_noise_ratio(image, C, data_range=255))
			BUFFER['BRAHIMI_Planar_Round']['SSIM'].append(structural_similarity(image, C, data_range=255))
			BUFFER['BRAHIMI_Planar_Round']['BPP'].append(bpp(BrahimiPrime2))
			
			QPhiRaizaForward = np2_round(divide(QOliveira, ZR_tiled))
			QPhiRaizaBackward = np2_round(multiply(QOliveira, ZR_tiled))
			...
			BUFFER['RAIZA_Planar_Round']['PSNR'].append(peak_signal_noise_ratio(image, D, data_range=255))
			BUFFER['RAIZA_Planar_Round']['SSIM'].append(structural_similarity(image, D, data_range=255))
			BUFFER['RAIZA_Planar_Round']['BPP'].append(bpp(RaizaPrime2))
			...
			BUFFER['OLIVEIRA_Planar_Round']['PSNR'].append(peak_signal_noise_ratio(image, E, data_range=255))
			BUFFER['OLIVEIRA_Planar_Round']['SSIM'].append(structural_similarity(image, E, data_range=255))
			BUFFER['OLIVEIRA_Planar_Round']['BPP'].append(bpp(OliveiraPlanarPrime2))

			# ---------------------------------------------------------------------

			QPhiBrahimiForward = np2_ceil(divide(QOliveira, ZB_tiled))
			QPhiBrahimiBackward = np2_ceil(multiply(QOliveira, ZB_tiled))
			BrahimiPrime2 = multiply(around(divide(BrahimiPrime1, QPhiBrahimiForward)), QPhiBrahimiBackward)
			BrahimiPrime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TB.T, BrahimiPrime2), TB)
			F = clip(Tools.remount(BrahimiPrime3, (h, w)), 0, 255)
			BrahimiPrime2 = BrahimiPrime2.reshape(h, w)
			BUFFER['BRAHIMI_Planar_Ceil']['PSNR'].append(peak_signal_noise_ratio(image, F, data_range=255))
			BUFFER['BRAHIMI_Planar_Ceil']['SSIM'].append(structural_similarity(image, F, data_range=255))
			BUFFER['BRAHIMI_Planar_Ceil']['BPP'].append(bpp(BrahimiPrime2))
			
			QPhiRaizaForward = np2_ceil(divide(QOliveira, ZR_tiled))
			QPhiRaizaBackward = np2_ceil(multiply(QOliveira, ZR_tiled))
			RaizaPrime2 = multiply(around(divide(RaizaPrime1, QPhiRaizaForward)), QPhiRaizaBackward)
			RaizaPrime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TR.T, RaizaPrime2), TR)
			G = clip(Tools.remount(RaizaPrime3, (h, w)), 0, 255)
			RaizaPrime2 = RaizaPrime2.reshape(h, w)
			BUFFER['RAIZA_Planar_Ceil']['PSNR'].append(peak_signal_noise_ratio(image, G, data_range=255))
			BUFFER['RAIZA_Planar_Ceil']['SSIM'].append(structural_similarity(image, G, data_range=255))
			BUFFER['RAIZA_Planar_Ceil']['BPP'].append(bpp(RaizaPrime2))

			QPhiOliveiraPlanarForward = np2_ceil(divide(QOliveira, ZO_tiled))
			QPhiOliveiraPlanarBackward = np2_ceil(multiply(QOliveira, ZO_tiled))
			OliveiraPlanarPrime2 = multiply(around(divide(OliveiraPrime1, QPhiOliveiraPlanarForward)), QPhiOliveiraPlanarBackward)
			OliveiraPlanarPrime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TO.T, OliveiraPlanarPrime2), TO)
			H = clip(Tools.remount(OliveiraPlanarPrime3, (h, w)), 0, 255)
			OliveiraPlanarPrime2 = OliveiraPlanarPrime2.reshape(h, w)
			BUFFER['OLIVEIRA_Planar_Ceil']['PSNR'].append(peak_signal_noise_ratio(image, H, data_range=255))
			BUFFER['OLIVEIRA_Planar_Ceil']['SSIM'].append(structural_similarity(image, H, data_range=255))
			BUFFER['OLIVEIRA_Planar_Ceil']['BPP'].append(bpp(OliveiraPlanarPrime2))
		...
		results.append({'File name':file, 'Method':"Brahimi Planar (R)", 'PSNR':BUFFER['BRAHIMI_Planar_Round']['PSNR'], 'SSIM':BUFFER['BRAHIMI_Planar_Round']['SSIM'], 'BPP':BUFFER['BRAHIMI_Planar_Round']['BPP']})
		results.append({'File name':file, 'Method':"Oliveira Planar (R)", 'PSNR':BUFFER['OLIVEIRA_Planar_Round']['PSNR'], 'SSIM':BUFFER['OLIVEIRA_Planar_Round']['SSIM'], 'BPP':BUFFER['OLIVEIRA_Planar_Round']['BPP']})
		results.append({'File name':file, 'Method':"Raiza Planar (R)", 'PSNR':BUFFER['RAIZA_Planar_Round']['PSNR'], 'SSIM':BUFFER['RAIZA_Planar_Round']['SSIM'], 'BPP':BUFFER['RAIZA_Planar_Round']['BPP']})
		results.append({'File name':file, 'Method':"Brahimi Planar (C)", 'PSNR':BUFFER['BRAHIMI_Planar_Ceil']['PSNR'], 'SSIM':BUFFER['BRAHIMI_Planar_Ceil']['SSIM'], 'BPP':BUFFER['BRAHIMI_Planar_Ceil']['BPP']})
		results.append({'File name':file, 'Method':"Oliveira Planar (C)", 'PSNR':BUFFER['OLIVEIRA_Planar_Ceil']['PSNR'], 'SSIM':BUFFER['OLIVEIRA_Planar_Ceil']['SSIM'], 'BPP':BUFFER['OLIVEIRA_Planar_Ceil']['BPP']})
		results.append({'File name':file, 'Method':"Raiza Planar (C)", 'PSNR':BUFFER['RAIZA_Planar_Ceil']['PSNR'], 'SSIM':BUFFER['RAIZA_Planar_Ceil']['SSIM'], 'BPP':BUFFER['RAIZA_Planar_Ceil']['BPP']})

results = sorted(results, key=itemgetter('File name'))
fieldnames = ['File name', 'Method', 'PSNR', 'SSIM', 'BPP']
with open('JPEG_Brahimi_Raiza_RDCT_Planar_with_np2.csv', 'w') as csv_file:
