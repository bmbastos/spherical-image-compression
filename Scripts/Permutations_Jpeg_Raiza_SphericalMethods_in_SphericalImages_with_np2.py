...
for file in tqdm(files):
	...
	if os.path.isfile(full_path) or processed_images < target:
		...
		for QF in quantization_factor:
			...
			# Raizas ESFÉRICOS com np2
			# Matematicamente correto
			QPhiRaizaSphericalForward = np2_round(divide(QPhiOliveira, ZR_tiled))
			QPhiRaizaSphericalBackward = np2_round(multiply(QPhiOliveira, ZR_tiled))
			RaizaSphericalPrime2 = multiply(around(divide(RaizaPrime1, QPhiRaizaSphericalForward)), QPhiRaizaSphericalBackward)
			RaizaPlanarPrime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TR.T, RaizaSphericalPrime2), TR)
			C = clip(Tools.remount(RaizaPlanarPrime3, (h, w)), 0, 255)
			...
			# Raiza with QPhi (Aplicação de QPhi em np2 de q_forward)
			...
			D = clip(Tools.remount(RaizaPermutation1Prime3, (h, w)), 0, 255)
			...
			BUFFER['Permutacao1']['BPP'].append(bpp(RaizaPermutation1Prime2))
			...
			RaizaPermutation2Prime2 = multiply(around(divide(RaizaPrime1, QPhiRaizaForward)), QPhiRaizaBackward)
			RaizaPermutation2Prime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TR.T, RaizaPermutation2Prime2), TR)
			E = clip(Tools.remount(RaizaPermutation2Prime3, (h, w)), 0, 255)
			RaizaPermutation2Prime2 = RaizaPermutation2Prime2.reshape(h, w)
			BUFFER['Permutacao2']['PSNR'].append(WSPSNR(image, E))
			BUFFER['Permutacao2']['SSIM'].append(WSSSIM(image, E))
			BUFFER['Permutacao2']['BPP'].append(bpp(RaizaPermutation2Prime2))
		...
		results.append({'File name':file, 'Method':r"$\operatorname{np2}((\mathbf{Q})_\phi\bigstar\mathbf{ZR})$", 'PSNR':BUFFER['RAIZA_Spherical']['PSNR'], 'SSIM':BUFFER['RAIZA_Spherical']['SSIM'], 'BPP':BUFFER['RAIZA_Spherical']['BPP']})
		results.append({'File name':file, 'Method':r"$\operatorname{np2}((\mathbf{Q}\bigstar\mathbf{ZR})_\phi)$", 'PSNR':BUFFER['Permutacao1']['PSNR'], 'SSIM':BUFFER['Permutacao1']['SSIM'], 'BPP':BUFFER['Permutacao1']['BPP']})
		results.append({'File name':file, 'Method':r"$(\operatorname{np2}(\mathbf{Q})\bigstar\mathbf{ZR})_\phi$", 'PSNR':BUFFER['Permutacao2']['PSNR'], 'SSIM':BUFFER['Permutacao2']['SSIM'], 'BPP':BUFFER['Permutacao2']['BPP']})
