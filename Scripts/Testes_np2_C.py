	...
	while processed_images < target:
		...
		if os.path.isfile(full_path):
			...
			BUFFER = {'JPEG_Spherical': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'RA_Q_CR_S_LowCost': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'OL_Q_CR_S_LowCost': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'RA_Q_CC_S_LowCost': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'OL_Q_CC_S_LowCost': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'RA_Q_CF_S_LowCost': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'OL_Q_CF_S_LowCost': {'PSNR':[], 'SSIM':[], 'BPP':[]}}
			...
			for QF in tqdm(quantization_factor):
				...
				# Raiza ------------------------------------------------------------------------------------------------------------------------------------- R
				QPhiRaizaForward = prepareQPhi(image, np2_ceil(divide(QOliveira, ZR)))
				...
				BUFFER['RA_Q_CR_S_LowCost']['PSNR'].append(WSPSNR(image, C))
				BUFFER['RA_Q_CR_S_LowCost']['SSIM'].append(WSSSIM(image, C))
				BUFFER['RA_Q_CR_S_LowCost']['BPP'].append(bpp(RaizaPermutation1Prime2))
				
				# RDCT
				QPhiOliveiraForward = prepareQPhi(image, np2_ceil(divide(QOliveira, ZO)))
				...
				BUFFER['OL_Q_CR_S_LowCost']['PSNR'].append(WSPSNR(image, D))
				BUFFER['OL_Q_CR_S_LowCost']['SSIM'].append(WSSSIM(image, D))
				BUFFER['OL_Q_CR_S_LowCost']['BPP'].append(bpp(OliveiraPrime2))

				# Raiza ------------------------------------------------------------------------------------------------------------------------------------- C
				QPhiRaizaForward = prepareQPhi(image, np2_ceil(divide(QOliveira, ZR)))
				QPhiRaizaBackward = prepareQPhi(image, np2_ceil(multiply(QOliveira, ZR)))
				RaizaPermutation1Prime2 = multiply(around(divide(RaizaPrime1, QPhiRaizaForward)), QPhiRaizaBackward)
				RaizaPermutation1Prime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TR.T, RaizaPermutation1Prime2), TR)
				E = clip(Tools.remount(RaizaPermutation1Prime3, (h, w)), 0, 255)
				RaizaPermutation1Prime2 = RaizaPermutation1Prime2.reshape(h, w)
				BUFFER['RA_Q_CC_S_LowCost']['PSNR'].append(WSPSNR(image, E))
				BUFFER['RA_Q_CC_S_LowCost']['SSIM'].append(WSSSIM(image, E))
				BUFFER['RA_Q_CC_S_LowCost']['BPP'].append(bpp(RaizaPermutation1Prime2))
				...
				OliveiraPrime2 = OliveiraPrime2.reshape(h, w)
				BUFFER['OL_Q_CC_S_LowCost']['PSNR'].append(WSPSNR(image, F))
				BUFFER['OL_Q_CC_S_LowCost']['SSIM'].append(WSSSIM(image, F))
				BUFFER['OL_Q_CC_S_LowCost']['BPP'].append(bpp(OliveiraPrime2))

				# Raiza ------------------------------------------------------------------------------------------------------------------------------------- F
				QPhiRaizaForward = prepareQPhi(image, np2_ceil(divide(QOliveira, ZR)))
				QPhiRaizaBackward = prepareQPhi(image, np2_floor(multiply(QOliveira, ZR)))
				RaizaPermutation1Prime2 = multiply(around(divide(RaizaPrime1, QPhiRaizaForward)), QPhiRaizaBackward)
				RaizaPermutation1Prime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TR.T, RaizaPermutation1Prime2), TR)
				G = clip(Tools.remount(RaizaPermutation1Prime3, (h, w)), 0, 255)
				RaizaPermutation1Prime2 = RaizaPermutation1Prime2.reshape(h, w)
				BUFFER['RA_Q_CF_S_LowCost']['PSNR'].append(WSPSNR(image, G))
				BUFFER['RA_Q_CF_S_LowCost']['SSIM'].append(WSSSIM(image, G))
				BUFFER['RA_Q_CF_S_LowCost']['BPP'].append(bpp(RaizaPermutation1Prime2))
				...
				OliveiraPrime2 = OliveiraPrime2.reshape(h, w)
				BUFFER['OL_Q_CF_S_LowCost']['PSNR'].append(WSPSNR(image, H))
				BUFFER['OL_Q_CF_S_LowCost']['SSIM'].append(WSSSIM(image, H))
				BUFFER['OL_Q_CF_S_LowCost']['BPP'].append(bpp(OliveiraPrime2))
			...
			results.append({'File name':file, 'Method':"Raiza-CR", 'PSNR':BUFFER['RA_Q_CR_S_LowCost']['PSNR'], 'SSIM':BUFFER['RA_Q_CR_S_LowCost']['SSIM'], 'BPP':BUFFER['RA_Q_CR_S_LowCost']['BPP']})
			results.append({'File name':file, 'Method':"Oliveira-CR", 'PSNR':BUFFER['OL_Q_CR_S_LowCost']['PSNR'], 'SSIM':BUFFER['OL_Q_CR_S_LowCost']['SSIM'], 'BPP':BUFFER['OL_Q_CR_S_LowCost']['BPP']})
			results.append({'File name':file, 'Method':"Raiza-CC", 'PSNR':BUFFER['RA_Q_CC_S_LowCost']['PSNR'], 'SSIM':BUFFER['RA_Q_CC_S_LowCost']['SSIM'], 'BPP':BUFFER['RA_Q_CC_S_LowCost']['BPP']})
			results.append({'File name':file, 'Method':"Oliveira-CC", 'PSNR':BUFFER['OL_Q_CC_S_LowCost']['PSNR'], 'SSIM':BUFFER['OL_Q_CC_S_LowCost']['SSIM'], 'BPP':BUFFER['OL_Q_CC_S_LowCost']['BPP']})
			results.append({'File name':file, 'Method':"Raiza-CF", 'PSNR':BUFFER['RA_Q_CF_S_LowCost']['PSNR'], 'SSIM':BUFFER['RA_Q_CF_S_LowCost']['SSIM'], 'BPP':BUFFER['RA_Q_CF_S_LowCost']['BPP']})
			results.append({'File name':file, 'Method':"Oliveira-CF", 'PSNR':BUFFER['OL_Q_CF_S_LowCost']['PSNR'], 'SSIM':BUFFER['OL_Q_CF_S_LowCost']['SSIM'], 'BPP':BUFFER['OL_Q_CF_S_LowCost']['BPP']})
			
results = sorted(results, key=itemgetter('File name'))
fieldnames = ['File name', 'Method', 'PSNR', 'SSIM', 'BPP']
with open('Teste_de_np2_ceiled_forward_with_LowCost.csv', 'w') as csv_file:
