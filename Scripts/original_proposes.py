...
for file in tqdm(files):
	...
	if os.path.isfile(full_path) or processed_images < target:
		...
		for QF in quantization_factor:
			...
			# RDCT, BRAHIMI, RAIZA E DE SIMONE
			...
			QPhiForward = divide(QPhiOliveira, ZO_tiled)
			QPhiBackward = multiply(QPhiOliveira, ZO_tiled)
			PhiPrime2 = multiply(around(divide(OliveiraPrime1, QPhiForward)), QPhiBackward)
...
with open('original_proposes2.csv', 'w') as csv_file:
