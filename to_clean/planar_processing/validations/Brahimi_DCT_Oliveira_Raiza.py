...
#QUANTIZATION = {'RDCT': {'Max_forward': [], 'Min_forward': [], 'Max_backward': [], 'Min_backward': []}, 'BRAHIMI': {'Max_forward': [], 'Min_forward': [], 'Max_backward': [], 'Min_backward': []}}
...
for file in tqdm(files):
	...
	if os.path.isfile(full_path):
		...
		for QF in quantization_factor:
			...
			BUFFER['JPEG_Planar']['PSNR'].append(float(peak_signal_noise_ratio(image, B, data_range=255)))
			BUFFER['JPEG_Planar']['SSIM'].append(float(structural_similarity(image, B, data_range=255)))
			...
			f_max = QOliveiraForward.max()
			b_max = QOliveiraBackward.max()
			#QUANTIZATION['RDCT']['Max_forward'].append(QOliveiraForward.max())
			#QUANTIZATION['RDCT']['Min_forward'].append(QOliveiraForward.min())
			#QUANTIZATION['RDCT']['Max_backward'].append(QOliveiraBackward.max())
			#QUANTIZATION['RDCT']['Min_backward'].append(QOliveiraBackward.min())
			...
			BUFFER['OLIVEIRA_Planar']['PSNR'].append(float(peak_signal_noise_ratio(image, C, data_range=255)))
			BUFFER['OLIVEIRA_Planar']['SSIM'].append(float(structural_similarity(image, C, data_range=255)))
			BUFFER['OLIVEIRA_Planar']['BPP'].append(bpp(OliveiraPrime2))


			# Brahimi
			QBrahimiForward = clip(np2_ceil(divide(QBrahimi, ZB_tiled)), 0, f_max)
			QBrahimiBackward = clip(np2_ceil(multiply(QBrahimi, ZB_tiled)), 0, b_max)
			
			#QUANTIZATION['BRAHIMI']['Max_forward'].append(QBrahimiForward.max())
			#QUANTIZATION['BRAHIMI']['Min_forward'].append(QBrahimiForward.min())
			#QUANTIZATION['BRAHIMI']['Max_backward'].append(QBrahimiBackward.max())
			#QUANTIZATION['BRAHIMI']['Min_backward'].append(QBrahimiBackward.min())
			...
			BUFFER['BRAHIMI_Planar']['PSNR'].append(float(peak_signal_noise_ratio(image, D, data_range=255)))
			BUFFER['BRAHIMI_Planar']['SSIM'].append(float(structural_similarity(image, D, data_range=255)))
			BUFFER['BRAHIMI_Planar']['BPP'].append(bpp(BrahimiPrime2))
			
			#print(QF)
			#print(f'RDCT {QOliveiraForward.max()} {QOliveiraForward.min()} {QOliveiraBackward.max()} {QOliveiraBackward.min()}')
			#print(f'Brahimi {QBrahimiForward.max()} {QBrahimiForward.min()} {QBrahimiBackward.max()} {QBrahimiBackward.min()}')
		...
		# Plotting the quantization coefficients
		QF_values = list(quantization_factor)
...
#print("Máximos")
#print(f"Forward: {max(QUANTIZATION['RDCT']['Max_forward'])}")
#print(f"Backward: {max(QUANTIZATION['RDCT']['Max_backward'])}")
#pause()

results = sorted(results, key=itemgetter('File name'))
fieldnames = ['File name', 'Method', 'PSNR', 'SSIM', 'BPP']
with open('Brahimi_FAdjusted_BAdjusted.csv', 'w') as csv_file:
