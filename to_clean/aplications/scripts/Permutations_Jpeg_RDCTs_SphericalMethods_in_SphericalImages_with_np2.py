...
for file in tqdm(files):
	...
	if os.path.isfile(full_path) or processed_images < target:
		...
		for QF in quantization_factor:
			...
			OliveiraSphericalPrime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TO.T, OliveiraSphericalPrime2), TO)
			C = clip(Tools.remount(OliveiraSphericalPrime3, (h, w)), 0, 255)
			...
			QPhiOliveiraP1Forward = prepareQPhi(image, np2_round(divide(QOliveira, ZO)))
			QPhiOliveiraP1Backward = prepareQPhi(image, np2_round(multiply(QOliveira, ZO)))
			OliveiraP1Prime2 = multiply(around(divide(OliveiraPrime1, QPhiOliveiraP1Forward)), QPhiOliveiraP1Backward)
			OliveiraP1Prime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TO.T, OliveiraP1Prime2), TO)
			D = clip(Tools.remount(OliveiraP1Prime3, (h, w)), 0, 255)
			OliveiraP1Prime2 = OliveiraP1Prime2.reshape(h, w)
			BUFFER['Permutacao1']['PSNR'].append(WSPSNR(image, D))
			BUFFER['Permutacao1']['SSIM'].append(WSSSIM(image, D))
			BUFFER['Permutacao1']['BPP'].append(bpp(OliveiraP1Prime2))
			
			# Oliveira with QPhi (Aplicação de Np2 em QPhi de q_forward)
			QPhiOliveiraP2Forward = np2_round(prepareQPhi(image, divide(QOliveira, ZO)))
			QPhiOliveiraP2Backward = np2_round(prepareQPhi(image, multiply(QOliveira, ZO)))
			OliveiraP2Prime2 = multiply(around(divide(OliveiraPrime1, QPhiOliveiraP2Forward)), QPhiOliveiraP2Backward)
			OliveiraP2Prime3 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TO.T, OliveiraP2Prime2), TO)
			E = clip(Tools.remount(OliveiraP2Prime3, (h, w)), 0, 255)
			OliveiraP2Prime2 = OliveiraP2Prime2.reshape(h, w)
			BUFFER['Permutacao2']['PSNR'].append(WSPSNR(image, E))
			BUFFER['Permutacao2']['SSIM'].append(WSSSIM(image, E))
			BUFFER['Permutacao2']['BPP'].append(bpp(OliveiraP2Prime2))
