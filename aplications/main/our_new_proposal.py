	...
	for file in files:
		if not os.path.isdir(partial_path + file) and file.endswith(".bmp"):
			...
			for QF in quantization_factor:
				...
				QPhiP2Forward = np2_round(divide(prepareQPhi(image, QOliveira), ZR_tiled))
				QPhiP2Backward = np2_round(multiply(prepareQPhi(image, QOliveira), ZR_tiled))
