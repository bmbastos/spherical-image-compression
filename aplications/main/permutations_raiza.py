...
for file in files:
	if not os.path.isdir(path_images + file) and file.endswith(".bmp"):
		...
		for QF in quantization_factor:
			...
			Prime1 = einsum('mij, jk -> mik', einsum('ij, mjk -> mik', TR, A), TR.T)
			
			# P2 = np2(adj(phi(Q), Z))
			QP2Forward = np2_round(divide(prepareQPhi(image, QOliveira), ZR_tiled))
			QP2Backward = np2_round(multiply(prepareQPhi(image, QOliveira), ZR_tiled))
			...
			# P3 = phi(np2(adj(Q, Z)))
			QP3Forward = prepareQPhi(image, np2_round(divide(QOliveira, ZR)))
			QP3Backward = prepareQPhi(image, np2_round(multiply(QOliveira, ZR)))
			...
			# P4 = np2(phi(adj(Q, Z)))
			QP4Forward = np2_round(prepareQPhi(image, divide(QOliveira, ZR)))
			QP4Backward = np2_round(prepareQPhi(image, multiply(QOliveira, ZR)))
...
with open(destination + 'permutations_raiza_4K.csv', 'w') as csv_file_4k:
