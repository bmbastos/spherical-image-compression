...
for directory in directories:
	...
	for file in files:
		if not os.path.isdir(partial_path + file) and file.endswith(".bmp"):
			...
			BUFFER = {'BRAHIMI1': {'PSNR':[], 'SSIM':[], 'BPP':[]},
						'BRAHIMI2': {'PSNR':[], 'SSIM':[], 'BPP':[]}}
			...
			for QF in quantization_factor:
				...
				QBrahimiForward = prepareQPhi(image, np2_ceil(divide(QBrahimi, ZB)))
				QBrahimiBackward = prepareQPhi(image, np2_ceil(multiply(QBrahimi, ZB)))
				...
				BUFFER['BRAHIMI1']['PSNR'].append(WSPSNR(image, F))
				BUFFER['BRAHIMI1']['SSIM'].append(WSSSIM(image, F))
				BUFFER['BRAHIMI1']['BPP'].append(bpp(BrahimiPrime2))
				del BrahimiPrime1; del BrahimiPrime2; del BrahimiPrime3; del F; del QBrahimiForward; del QBrahimiBackward
				'''Proposta original da Brahimi'''
				...
				F = clip(Tools.remount(BrahimiPrime3, (h, w)), 0, 255)
				BrahimiPrime2 = BrahimiPrime2.reshape(h, w)
				BUFFER['BRAHIMI2']['PSNR'].append(WSPSNR(image, F))
				BUFFER['BRAHIMI2']['SSIM'].append(WSSSIM(image, F))
				BUFFER['BRAHIMI2']['BPP'].append(bpp(BrahimiPrime2))
				del BrahimiPrime1; del BrahimiPrime2; del BrahimiPrime3; del F; del QBrahimiForward; del QBrahimiBackward; del QBrahimi
				'''Proposta original da Brahimi com a mudança somente da função de np2'''
			
			processed_images += 1
			results.append({'File name':file, 'Method':r"$\operatorname{np2}^{\uparrow}(\mathbf{Q}^{B})$", 'PSNR':BUFFER['BRAHIMI1']['PSNR'], 'SSIM':BUFFER['BRAHIMI1']['SSIM'], 'BPP':BUFFER['BRAHIMI1']['BPP']})
			results.append({'File name':file, 'Method':r"$\operatorname{np2}^{\circ}(\mathbf{Q}^{B})$", 'PSNR':BUFFER['BRAHIMI2']['PSNR'], 'SSIM':BUFFER['BRAHIMI2']['SSIM'], 'BPP':BUFFER['BRAHIMI2']['BPP']})
			'''
...
with open(destination + 'brahimi_4K.csv', 'w') as csv_file_4k:
