...
for file in tqdm(files):
	...
	if os.path.isfile(full_path):
		...
		for index, QF in enumerate(quality_factors):
			...
			# Proposta do Oliveira (TO|QB|NP2ROUND)
			PO2_Q_forward = tile(asarray([divide(QBrahimiCeiled, ZOliveira)]), (Aprime1Oliveira.shape[0], 1, 1))
			PO2_Q_backward = tile(asarray([multiply(QBrahimiCeiled, ZOliveira)]), (Aprime1Oliveira.shape[0], 1, 1))
			...
			# Proposta da Brahimi (TB|QO|NP2CEIL)
			PB2_Q_forward = tile(asarray([divide(QOliveiraRounded, ZBrahimi)]), (Aprime1Brahimi.shape[0], 1, 1))
			PB2_Q_backward = tile(asarray([multiply(QOliveiraRounded, ZBrahimi)]), (Aprime1Brahimi.shape[0], 1, 1))
