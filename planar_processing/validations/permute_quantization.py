...
for file in tqdm(files):
	...
	if os.path.isfile(full_path):
		...
		for index, QF in enumerate(quality_factors):
			...
			# Proposta do Oliveira (TO|QB|NP2ROUND) permutada
			PO2_Q_forward = tile(asarray([np2_round(divide(QBrahimi, ZOliveira))]), (Aprime1Oliveira.shape[0], 1, 1))
			PO2_Q_backward = tile(asarray([np2_round(multiply(QBrahimi, ZOliveira))]), (Aprime1Oliveira.shape[0], 1, 1))
			...
			# Proposta da Brahimi (TB|QO|NP2CEIL) permutada
			PB2_Q_forward = tile(asarray([np2_ceil(divide(QOliveira, ZBrahimi))]), (Aprime1Brahimi.shape[0], 1, 1))
			PB2_Q_backward = tile(asarray([np2_ceil(multiply(QOliveira, ZBrahimi))]), (Aprime1Brahimi.shape[0], 1, 1))
		...
		results.append({'File name':file, 'Method':'Oliveira with QB', 'PSNR':BUFFER['PO2']['PSNR'], 'SSIM':BUFFER['PO2']['SSIM'], 'BPP':BUFFER['PO2']['BPP']})
		results.append({'File name':file, 'Method':'Brahimi propose', 'PSNR':BUFFER['PB1']['PSNR'], 'SSIM':BUFFER['PB1']['SSIM'], 'BPP':BUFFER['PB1']['BPP']})
		results.append({'File name':file, 'Method':'Brahimi with QO', 'PSNR':BUFFER['PB2']['PSNR'], 'SSIM':BUFFER['PB2']['SSIM'], 'BPP':BUFFER['PB2']['BPP']})

		n_images += 1

results = sorted(results, key=itemgetter('File name'))
fieldnames = ['File name', 'Method', 'PSNR', 'SSIM', 'BPP']
with open('permute_quantization.csv', 'w') as csv_file:
